...
void renderConsole() {
	...
	for (int i = 0; i < player.size(); i++) {
		cout << "Player " << i << ": ";
		cout << player[i].x << " " << player[i].y << endl;
	}
}
...
bool validPlayerPosition(const int& x, const int& y) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				...
				if (i == 0) xMap = x - cellWidth/2;
				else xMap = x + cellWidth/2 - 1;

				if (j == 0) yMap = y - cellHeight/2;
				else yMap = y + cellHeight/2 - 1;
...
bool validInput(const tEvent& event) {
	if (event.type == "direction") {
		...
		if (!validPlayerPosition(event.x, event.y)) {
			return false;
		}
...
bool checkFourCorner(const int& xWindow, const int& yWindow, const int& xMap, const int& yMap) {
	int xCorner, yCorner;
...
void updateGoThrough(vector<tBomb>& bomb) {
	for (int i = 0; i < bomb.size(); i++) {
		if (checkFourCorner(player[bomb[i].owner].x, player[bomb[i].owner].y, bomb[i].x, bomb[i].y)) {
...
void updateGameStatus() {
		...
		for (int i = 0; i < player.size(); i++) {
			if (player[i].direction.size() > 0) {
				...
				int newX = player[i].x + directX[k] * player[i].speed;
				int newY = player[i].y + directY[k] * player[i].speed;
				...
				if (validPlayerPosition(newX, newY)) {
					updateUserCoordinate(i, newX, newY); 	
					processItem(i, newX, newY);
				}
			}
		}
