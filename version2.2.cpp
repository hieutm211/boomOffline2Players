...
void bind(const tMap& map, const vector<tPlayer>& player, const vector<tBomb>& bomb, const vector<tItem>& item, vector<vector<char>>& mapBind) {
	...
	for (int i = 0; i < bomb.size(); i++) {
		...
		if (bombInProcess(bomb[i])) {
			...
			for (int i = max(0, x - bombSize); i < min(map.row-1, x + bombSize)+1; i++) {
				mapBind[i][y] = bombChar;
			}
			for (int j = max(0, y - bombSize); j < min(map.col-1, y + bombSize)+1; j++) {
...
void removeExplodedBomb(vector<tBomb>& bomb) {
	...
	int i = 0;
	while (i < bomb.size() && difftime(timeNow, bomb[i].timeStart) > map.bombDuration + map.bombProcessTime) {
		i++;		
	} 
	
	bomb.assign(bomb.begin()+i, bomb.end());
}
...
bool inBombLine(const int& x, const int& y) {
	...
	for (int i = 0; i < bomb.size(); i++) {
		if (bombInProcess(bomb[i])) {
			...
			if (y == bomb[i].y && pointInLine(x, bomb[i].x - bombSize, bomb[i].x + bombSize)) return true;
			if (x == bomb[i].x && pointInLine(y, bomb[i].y - bombSize, bomb[i].y + bombSize)) return true;
		}
	}
	return false;
}
