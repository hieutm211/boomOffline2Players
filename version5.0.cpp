...
void readMap(const string& mapFileName) {
	...
	mapFile >> mapData.row >> mapData.col >> mapData.gameDuration 
		>> mapData.maxBomb >> mapData.maxBombSize >> mapData.bombDuration >> mapData.bombProcessTime 
		>> mapData.maxSpeed >> mapData.itemDelay;
...
bool addAnEvent() {
	return ((int) difftime(time(NULL), mapData.startTime) % mapData.itemDelay == 0); 
}
